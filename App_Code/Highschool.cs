...
public class Highschool : WebService
{
    public Highschool()
    {
    }
    ...
    public string[] GetCompletionList(string prefixText, int count, string contextKey)
    {
        ...
        string strSQL = "SELECT Name FROM cc_masterdataset.institution WHERE (Name like '" + prefixText + "%')AND (Country_ID = " + Convert.ToInt32(country.ToString()) + ") AND (Type = 'HighSchool') AND (Confirmed = 1) AND ((Customer_Id=" + Convert.ToInt32(customer.ToString()) + ") or (Customer_Id=0))";                        
